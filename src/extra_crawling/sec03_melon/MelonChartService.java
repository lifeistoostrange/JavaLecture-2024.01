...
public class MelonChartService {
	...
	public List<MelonChart> getMellonChart() throws IOException {
		String url = "https://www.melon.com/chart/index.htm";
		Document doc = Jsoup.connect(url).get();
		Elements trs = doc.select(".service_list_song.type02.d_song_list > table > tbody > tr");
		
		List<MelonChart> chart = new ArrayList<MelonChart>();
		...
		for (Element tr : trs) {
			...
			MelonChart ch = new MelonChart(rank, title, artist, album, src);
			chart.add(ch);
		} 
		return chart;		
	}
